...
path converts(const std::string& str) {
	...
	result.disk = str.substr(0, pos_disk);
	result.users = str.substr(pos_disk + 1, pos_users - pos_disk - 1);
	result.user1 = str.substr(pos_users + 1, pos_username - pos_users - 1);
	result.programm = str.substr(pos_username + 1);
